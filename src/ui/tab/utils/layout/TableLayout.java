...
public class TableLayout implements java.awt.LayoutManager2, java.io.Serializable, TableLayoutConstants {
	...
	public boolean overlapping() {
		...
		for (int knowUnique = 1; knowUnique < numEntry; knowUnique++)
			for (int checking = knowUnique - 1; checking >= 0; checking--)
				if (((entry[checking].col1 >= entry[knowUnique].col1) && (entry[checking].col1 <= entry[knowUnique].col2) && (entry[checking].row1 >= entry[knowUnique].row1) && (entry[checking].row1 <= entry[knowUnique].row2))
						|| ((entry[checking].col2 >= entry[knowUnique].col1) && (entry[checking].col2 <= entry[knowUnique].col2)
			...
			if ((columnSpec[counter] == PREFERRED) || (columnSpec[counter] == MINIMUM)) {
				...
				while (iterator.hasNext()) {
					...
					if ((entry.col1 == counter) && (entry.col2 == counter)) {
						Dimension p = (columnSpec[counter] == PREFERRED) ? entry.component.getPreferredSize() : entry.component.getMinimumSize();
			...
			if ((rowSpec[counter] == PREFERRED) || (rowSpec[counter] == MINIMUM)) {
				...
				while (iterator.hasNext()) {
					...
					if ((entry.row1 == counter) && (entry.row2 == counter)) {
						Dimension p = (rowSpec[counter] == PREFERRED) ? entry.component.getPreferredSize() : entry.component.getMinimumSize();
	...
	public Dimension preferredLayoutSize(Container container) {
		...
		while (iterator.hasNext()) {
			...
			if ((entry.col1 < 0) || (entry.col1 >= columnSpec.length) || (entry.col2 >= columnSpec.length) || (entry.row1 < 0) || (entry.row1 >= rowSpec.length)
